		...
		<script>
			$(function(){
				$('.activo').on('change', function(){
					...
					$.ajax({
						url:'/administrador/estatus/red.html',
						type:'POST',
						data:{
							_token:"{{csrf_token()}}",
							id:id
			...
			<!-- START PANEL -->
				<div class="panel panel-transparent">
					<div class="panel-heading">
						<div class="panel-title">Redes Sociales
						</div>
						<div class="pull-right">
							<div class="col-xs-12">
		                    	<a href="{{url('/administrador/agregar/red.html')}}" class="btn btn-primary btn-cons"><i class="fa fa-plus"></i> Agregar</a>
			              	</div>
		                </div>
					</div>
					<div class="panel-group" id="accordion" role="tablist" aria-multiselectable="true">
						@foreach($redes as $val => $sucursal)
										...
										<div class="table-responsive">
											<div id="detailedTable_wrapper" class="dataTables_wrapper form-inline no-footer">
												<table class="table table-hover table-condensed table-detailed dataTable no-footer" id="detailedTable" role="grid">
													...
													<tbody>
														@foreach($sucursal as $red)
														<tr role="row" class="">
															<td class="v-align-middle">{{$red->red}}</td>
															<td class="v-align middle">{{$red->url}}</td>
															<td class="v-align-middle"><input type="checkbox" {{$red->estatus == 1 ? "checked" : ""}} class="activo" data="{{$red->id}}" data-toggle="toggle"></td>
															<td class="v-align-middle">
																<div class="btn-group btn-group-justified">
										                            <div class="btn-group">
										                              	<form action="{{url('/administrador/modificar/red' . $red->id . '.html')}}" method="post">
																			{!!csrf_field()!!}
										                            ...
										                            <div class="btn-group">
										                            	<form action="{{url('/administrador/eliminar/red' . $red->id . '.html')}}" method="post" class="del_element">
																			{!!csrf_field()!!}
