		...
		<!-- START PANEL -->
			<div class="panel panel-transparent">
		  		<div class="panel-body">
				    <form id="form-agregar" role="form" autocomplete="off" method="POST" action="{{url('/administrador/modificar/calendario' . $id . '.html')}}" enctype="multipart/form-data">
				    	{!!csrf_field()!!}
				    	...
				    	<div class="row clearfix">
				        	<div class="col-sm-12">
				          		<div class="form-group form-group-default" aria-required="true">
				            		...
				            		<input type="text" id="titulo" class="form-control required" value="{{$calendario->titulo}}" name="titulo" required="required" aria-required="true" aria-invalid="true">
				          		</div>
				        	</div>
				      	</div>
		                ...
		                <div class="row clearfix">
				        	<div class="col-sm-12">
				          		<div class="form-group form-group-default" aria-required="true">
				            		...
				            		<textarea id="descripcion" class="form-control required" name="descripcion" required="required" aria-required="true" aria-invalid="true">{{$calendario->descripcion}}</textarea>
				          		</div>
				        	</div>
				      	</div>
				            		...
				            		<select id="categoria" class="form-control required" name="categoria" required="required" aria-required="true" aria-invalid="true">
				            			...
				            			@if( isset($categorias) && count($categorias) )
				            				@foreach($categorias as $item)
				            					<option value="{{$item->id}}" {{$item->id == $calendario->id_categoria ? 'selected' : ''}}>{{$item->nombre}}</option>
				            				@endforeach
				            			@endif
				            		</select>
		        				...
		        				<div class="form-group form-group-default">
		        					<label for="sucursal">Sucursal</label>
					      			<select class="cs-select cs-skin-slide" data-init-plugin="cs-select" name="sucursal" id="sucursal">
				                      	...
				                      	<option value="{{$sucursal->id}}" {{$sucursal->id == $calendario->id_sucursal ? 'selected' : ''}}>{{$sucursal->nombre}}</option>
				                      	@endforeach
		                    		</select>
		                    	</div>
		                	</div>
		                ...
		                <div class="row clearfix">
				        	<div class="col-sm-12">
				          		<div class="form-group form-group-default" aria-required="true">
				            		...
				            		<input type="text" id="fecha" class="form-control required" value="{{date('Y-m-d',strtotime($calendario->inicio))}}" name="fecha" required="required" aria-required="true" aria-invalid="true">
				          		</div>
				        	</div>
				      	</div>
