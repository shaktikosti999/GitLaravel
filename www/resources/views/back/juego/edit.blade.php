	...
	@section('script')
		...
		$(function(){
			...
			$('#archivo').on('change',function(evt){
		 	...
		 	$('.img_juego_thumb').on('click', function(){
				$('#thumb').click();
			});
			$('#thumb').on('change',function(evt){
		 		$('.img_juego_thumb').attr('src', URL.createObjectURL(evt.target.files[0]));
		 	});
		});
		</script>
	@stop
			      	...
			      	<div class="row">
			        	<div class="col-sm-12">
			          		<div class="form-group form-group-default">
			            		...
			            		<img src="{{$juego->thumb}}" class="img-responsive img_juego_thumb">
			            		<input type="file" id="thumb" class="form-control required" name="thumb" aria-required="true" aria-invalid="true" style="display:none;">
			          		</div>
			          	</div>
			      	</div>
